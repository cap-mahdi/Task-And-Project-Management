...
import { EmailRoleInput, UpdateUserWorkspace, WorkspaceRole } from '../graphql';
...
export class UserWorkspaceResolver {
  ...
  async updateUserWorkspace(
    ...
    if (!(await this.userWorkspaceService.isAuthorized(user.id, workspaceId, roles))) {
    ...
    return this.userWorkspaceService.updateUserWorkspace(memberId, workspaceId, role);
  }
  ...
  async addUsersToWorkspace(
    ...
    return await Promise.all(
      emailRoles.map(async ({ email, role }) => {
        ...
        return this.userWorkspaceService.addUserToWorkspace(userToAdd.id, workspaceId, role);
      }))
  }
  ...
  async workspace(
    ...
    return this.workspaceService.findWorkspaceByUserWorkspaceId(userWorkspace.id);
  }
